...
(function() {
  var NONSENCE_CALLBACK, NOT_PASSABLE_HEAD_FIELS, OssEasy, UrlParser, assert, async, debuglog, fs, generateRandomId, ossAPI, path, wget, _;
  ...
  NOT_PASSABLE_HEAD_FIELS = ['content-Length', 'age', 'accept-ranges', 'connection', 'etag'];
  ...
  OssEasy = (function() {
    function OssEasy(ossOptions, targetBucket) {
      ...
      ossOptions.port || (ossOptions.port = 80);
      if (ossOptions.uploaderHeaders != null) {
        this.uploaderHeaders = ossOptions.uploaderHeaders;
        if (this.uploaderHeaders.contentType) {
          this.contentType = this.uploaderHeaders.contentType;
        }
    ...
    OssEasy.prototype.writeFile = function(remoteFilePath, data, headers, callback) {
      ...
      args = {
        ...
        srcFile: data,
        contentType: (headers && headers.contentType) || this.contentType || contentType
      };
    ...
    OssEasy.prototype.uploadFile = function(filePath, remoteFilePath, headers, callback) {
      var isLocal, options, pathToTempFile;
      if (_.isFunction(headers) && (callback == null)) {
        callback = headers;
        headers = null;
      }
      filePath = String(filePath || '');
      isLocal = (filePath.indexOf('http://') !== 0) && (filePath.indexOf('https://') !== 0);
      if (isLocal) {
        if (!fs.existsSync(filePath)) {
          return typeof callback === "function" ? callback("file not found. " + filePath) : void 0;
        }
        return this._uploadFileLocal(filePath, remoteFilePath, headers, callback);
      }
      pathToTempFile = path.join("/tmp/", generateRandomId());
      options = {
        url: filePath,
      ...
      wget(options, (function(_this) {
        return function(err, response) {
          var arr, contentType, key, value, _ref;
          if (err != null) {
          ...
          _this._uploadFileLocal(pathToTempFile, remoteFilePath, headers, callback);
        };
      })(this));
    };

    OssEasy.prototype._uploadFileLocal = function(localFilePath, remoteFilePath, headers, callback) {
      var args, timeKey;
      debuglog("[_uploadFileLocal] local:" + localFilePath + " -> " + this.targetBucket + ":" + remoteFilePath);
    ...
    OssEasy.prototype.transport = function(url, remoteFilePath, headers, callback) {
      var options, pathToTempFile;
      debuglog("[transport] url:" + url + " -> " + this.targetBucket + ":" + remoteFilePath);
      pathToTempFile = path.join("/tmp/", generateRandomId());
      options = {
        url: url,
        dest: pathToTempFile,
      ...
      wget(options, (function(_this) {
        return function(err) {
          ...
          _this.uploadFile(pathToTempFile, remoteFilePath, headers, callback);
        };
      })(this));
    };
    ...
    OssEasy.prototype.copyFolder = function(sourceFolderPath, destinationFolderPath, callback) {
      var args, err;
      debuglog("[copyFolder] source:" + sourceFolderPath + " destination:" + destinationFolderPath);
      if (!(_.isString(sourceFolderPath) && sourceFolderPath && destinationFolderPath && _.isString(destinationFolderPath))) {
      ...
      this.oss.listObject(args, (function(_this) {
        return function(err, result) {
          var des, item, key, tasks, _i, _len, _ref;
          if (err != null) {
            debuglog("ERROR [copyFolder] error:" + err);
            callback(err);
            return;
          ...
          _this.copyFiles(tasks, callback);
        };
      })(this));
    };

    OssEasy.prototype.setUploaderHeaders = function(uploaderHeaders) {
      return this.uploaderHeaders = uploaderHeaders;
    };

    OssEasy.prototype.setContentType = function(contentType) {
